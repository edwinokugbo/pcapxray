...
class PlotNetwork:
    ...
    def draw_graph(self, option="All", to_ip="All", from_ip="All"):
        ...
        interactive_graph = Network(directed=True, height="1200px", width="100vw", bgcolor="#222222", font_color="white")
        ...
        print("Starting Graph Plotting 22222")
        edge_present = False
        ...
        if option == "All":
            ...
            for packet in self.packets:
                src = packet.ether_src
                dst = packet.ether_dst
                port = packet.src_port
                session = src + '/' + dst + '/' + port
                ...
                if (src == from_ip and dst == to_ip) or \
                    ...
                    # Lan Host
                    if packet.ether_src not in self.lan_hosts:
                        curr_node = map_src+"\n"+packet.ether_src.replace(":",".")
                        f.node(curr_node)
                    else:
                        curr_node = self.lan_hosts[packet.ether_src]["node"]
                        f.node(curr_node)
                    ...
                    if dst in self.destination_hosts:
                        ...
                        if packet.ether_dst in self.lan_hosts:
                            destination = self.lan_hosts[packet.ether_dst]["node"]
                            dlabel = ""
                        else:
                            destination = packet.ether_dst.replace(":",".")
                            destination += "\n"+"PossibleGateway"
                            dlabel = ""
