...
import xray.libs.tor_traffic_handle as tor_traffic_handle
import xray.libs.malicious_traffic_identifier as malicious_traffic_identifier
        ...
        if option == "All":
            ...
            for packet in self.packets:
                ...
                if (src == from_ip and dst == to_ip) or \
                    ...
                    # Lan Host
                    if packet["Ethernet"]["src"] not in self.lan_hosts:
                        curr_node = map_src + "\n" + packet["Ethernet"]["src"].replace(":", ".")
                        f.node(curr_node)
                    else:
                        curr_node = self.lan_hosts[packet["Ethernet"]["src"]]["node"]
                        f.node(curr_node)

                    # Destination
                    if dst in self.destination_hosts:
                        if self.destination_hosts[dst]["mac"] in self.lan_hosts:
                            destination = self.lan_hosts[self.destination_hosts[dst]["mac"]]["node"]
                            dlabel = self.destination_hosts[dst]["domain_name"]
                        else:
                            destination = self.destination_hosts[dst]["mac"].replace(":", ".")
                            destination += "\n" + "PossibleGateway"
                            dlabel = self.destination_hosts[dst]["domain_name"]
                    else:
                        if packet["Ethernet"]["dst"] in self.lan_hosts:
                            destination = self.lan_hosts[packet["Ethernet"]["dst"]]["node"]
                            dlabel = ""
                        else:
                    ...
                    if curr_node != destination:
                        if session in self.possible_tor_traffic:
                            ...
                            interactive_graph.add_edge(curr_node, destination, color="white",
                                                       title='TOR: ' + str(map_dst),
                                                       smooth={"type": "curvedCW", "roundness": tor / 9})
                            ...
                            if port == "53":
                                ...
                                protocol = port
                            f.edge(curr_node, destination,
                                   label='Covert/' + protocol + ': ' + str(map_dst) + ": " + str(dlabel), color="cyan")
                            covert += 1
                            interactive_graph.add_edge(curr_node, destination, color="cyan",
                                                       title='Covert: ' + str(map_dst) + ": " + dlabel,
                                                       smooth={"type": "curvedCCW", "roundness": covert / 12})
                            if edge_present == False:
                                edge_present = True
                        else:
                            if port == "443":
                                f.edge(curr_node, destination, label='HTTPS: ' + str(map_dst) + ": " + str(dlabel),
                                       color="blue")
                                https += 1
                                interactive_graph.add_edge(curr_node, destination, color="blue",
                                                           title='HTTPS: ' + str(map_dst) + ": " + dlabel,
                                                           smooth={"type": "curvedCCW", "roundness": https / 10})

                                if edge_present == False:
                                    edge_present = True
                            elif port == "80":
                                f.edge(curr_node, destination, label='HTTP: ' + str(map_dst) + ": " + str(dlabel),
                                       color="green")
                                http += 1
                                interactive_graph.add_edge(curr_node, destination, color="green",
                                                           title='HTTP: ' + str(map_dst) + ": " + dlabel,
                                                           smooth={"type": "curvedCW", "roundness": http / 12})
                                ...
                                interactive_graph.add_edge(curr_node, destination, color="purple",
                                                           ...
                                                           smooth={"type": "curvedCCW", "roundness": icmp / 6})
                                ...
                                interactive_graph.add_edge(curr_node, destination, color="pink",
                                                           ...
                                                           smooth={"type": "curvedCW", "roundness": dns / 5})

                                if edge_present == False:
                                    edge_present = True
                            # elif int(port) in [20, 21, 23, 25, 110, 143, 139, 69, 161, 162, 1521]:
                            #     f.edge(curr_node, destination, label='ClearTextProtocol/' + port + ': ' + str(map_dst),
                            #            color="violet")
                            #     clear_text += 1
                            #     interactive_graph.add_edge(curr_node, destination, color="#9A2EFE",
                            #                                title='ClearTextProtocol/' + port + ': ' + str(map_dst),
                            #                                smooth={"type": "curvedCW", "roundness": clear_text / 4})
                            #     if edge_present == False:
                            #         edge_present = True
                            else:
                                pt = port
                        ...
                        unknown += 1
                        interactive_graph.add_edge(curr_node, other_node, color="pink", title='WeirdTraffic/' + port,
                                                   smooth={"type": "curvedCW", "roundness": unknown / 3})
                        if edge_present == False:
                            edge_present = True
            ...
            for packet in self.packets:
                ...
                if (src == from_ip and dst == to_ip) or \
                    ...
                    if port == "80" and curr_node != destination:
                        ...
                        http += 1
                        interactive_graph.add_edge(curr_node, destination, color="green",
                                                   smooth={"type": "curvedCW", "roundness": http / 10})
                        if edge_present == False:
                            edge_present = True
            ...
            for packet in self.packets:
                ...
                if (src == from_ip and dst == to_ip) or \
                    ...
                    if session in self.possible_tor_traffic and curr_node != destination:
                        f.edge(curr_node, destination, label='TOR: ' + str(map_dst), color="white")
                        tor += 1
                        interactive_graph.add_edge(curr_node, destination, color="white",
                                                   smooth={"type": "curvedCW", "roundness": tor / 10})
                        if edge_present == False:
                            edge_present = True
            ...
            for packet in self.packets:
                ...
                if (src == from_ip and dst == to_ip) or \
                    ...
                    if session in self.possible_mal_traffic and curr_node != destination:
                        ...
                        interactive_graph.add_edge(curr_node, destination, color="red",
                                                   smooth={"type": "curvedCW", "roundness": mal / 10})
                        if edge_present == False:
                            edge_present = True
            ...
            for packet in self.packets:
                ...
                if (src == from_ip and dst == to_ip) or \
                    ...
                    if port == "53" and curr_node != destination:
                        ...
                        dns += 1
                        interactive_graph.add_edge(curr_node, destination, color="pink",
                                                   smooth={"type": "curvedCW", "roundness": dns / 10})
                        if edge_present == False:
                            edge_present = True
